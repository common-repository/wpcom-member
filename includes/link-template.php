...
function wpcom_subpage_url( $subpage = '', $page = '' ){
    ...
    if($page_id){
        ...
        if($subpage === 'logout'){
            $url =  add_query_arg('nonce', wp_create_nonce('wpcom_logout'), $url);
        }
        return $url;
    }
}
...
function wpcom_social_login_url( $type, $action = 'login' ){
    ...
    $redirect_to = wpcom_login_redirect_to();
    if($redirect_to){
        \WPCOM\Themer\Session::set('redirect_to', $redirect_to);
...
function wpcom_login_redirect_to(){
    $options = $GLOBALS['wpmx_options'];
    $redirect_to = '';
    if(isset($_GET['redirect_to']) && $_GET['redirect_to']){
        $redirect_to = sanitize_url($_GET['redirect_to']);
    }else if(wp_doing_ajax() && (!isset($options['login_redirect']) || $options['login_redirect'] === '')){
